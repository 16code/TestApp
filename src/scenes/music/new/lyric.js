export default class Lyric {
    ...
    update(currentTime = 0) {
        ...
        const { lrcData = [], container, scrollBox } = this.options;
        ...
        if (
            ...
            for (let i = 0; i < lrcLen; i++) {
                if (currentTime >= lrcData[i][0] && (!lrcData[i + 1] || currentTime < lrcData[i + 1][0])) {
                    ...
                    scrollBox.style.transform = `translate3d(0, ${pos}rem, 0)`;
                    container.getElementsByClassName('lrc-on')[0].classList.remove('lrc-on');
                    container.getElementsByTagName('li')[i].classList.add('lrc-on');
                }
            }
        }
    }
    ...
    updateStyle() {
        ...
        const height = `${lrcLineHeight * (lrcMaxVisibleLine + 1)}rem`;
        const lineHeight = `${lrcLineHeight}rem`;
